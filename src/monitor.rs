...
mod platform {
    ...
    impl Monitor {
        pub fn set_vcp(&mut self, code: u8, value: u32) -> Result<(), String> {
            self.device.set_vcp_feature(code, value as u16)
                .map_err(|e| format!("set_vcp(0x{code:02X}, {value}) failed: {e:?}"))
        }

        pub fn get_vcp(&mut self, code: u8) -> Result<(u32, u32), String> {
            let reply = self.device.get_vcp_feature(code)
                .map_err(|e| format!("get_vcp(0x{code:02X}) failed: {e:?}"))?;
            Ok((reply.value() as u32, reply.maximum() as u32))
        }
        ...
        pub fn capabilities_string(&mut self) -> Result<String, String> {
            ...
            match self.device.capabilities(&mut buf) {
                Ok(len) => Ok(String::from_utf8_lossy(&buf[..len]).to_string()),
                Err(_) => Ok(String::new()),
            }
        }

        pub fn update_capabilities(&mut self) -> Result<(), String> {
            let caps = self.capabilities_string().unwrap_or_default();
            self.capabilities.raw_capabilities = caps.clone();
            self.capabilities.vcp_codes = parse_vcp_codes(&caps);
